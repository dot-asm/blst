...
.cfi_end_prologue
	...
	mov	$b_org, $b_ptr
	mov	$r_ptr, %r28		# put aside the arguments
	mov	$a_ptr, %r29
...
.cfi_end_prologue
	...
	mulx	@acc[6], @acc[0], @acc[1]
	call	__mula_mont_384
___
{
my @acc = map("%r$_","dx",23..27,	# output from __mula_mont_384
                     16..21);
$code.=<<___;
...
.cfi_end_prologue
	...
	sub	8*0($n_ptr), @acc[6], @acc[0]
	sbb	8*1($n_ptr), @acc[7], @acc[1]
	sbb	8*2($n_ptr), @acc[8], @acc[2]
	sbb	8*3($n_ptr), @acc[9], @acc[3]
	sbb	8*4($n_ptr), @acc[10], @acc[4]
	sbb	8*5($n_ptr), @acc[11], $a_ptr
...
.cfi_startproc
	...
	add	@acc[6], @acc[1]
	mulx	@acc[9], @acc[8], @acc[4]
	adc	@acc[7], @acc[2]
	mulx	@acc[10], $lo, @acc[5]
	adc	@acc[8], @acc[3]
	mulx	@acc[11], $hi, @acc[6]
	 mov	8($b_ptr), %rdx
	adc	$lo, @acc[4]
	adc	$hi, @acc[5]
	adc	\$0, @acc[6]
...
{nf}	imulq	$n0, %rdx
	...
	sub	8*0($n_ptr), @acc[1], %rdx
	sbb	8*1($n_ptr), @acc[2], @acc[8]
	sbb	8*2($n_ptr), @acc[3], @acc[9]
	sbb	8*3($n_ptr), @acc[4], @acc[10]
	sbb	8*4($n_ptr), @acc[5], $lo
	sbb	8*5($n_ptr), @acc[6], $hi
	sbb	\$0, @acc[7]
...
.Loop_sqra_384:
	...
	lea	-128($b_ptr), $a_ptr	# control u-op density
	lea	-128($n_ptr), $n_ptr	# control u-op density
	mov	%r9, $b_ptr		# 6th, multiplicand argument
...
.Loop_sqra_383:
	...
	mulx	%rdx, @acc[0], @acc[1]	# a[0]*a[0]
	call	__mula_mont_383_nonred	# omitting full reduction gives ~15%
					# in addition-chains
	mov	$r_ptr, $b_ptr
	dec	%r28d
	jnz	.Loop_sqra_383
...
$code.=<<___;
.type	__mula_mont_383_nonred,\@abi-omnipotent
.align	32
